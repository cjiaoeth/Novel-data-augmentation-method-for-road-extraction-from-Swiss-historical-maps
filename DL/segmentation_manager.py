...
from utils import raise_error, get_timestamp, ndshp2dict, trim_GeoJSON, sort_by_key,ndshp2dict_DA,trim_GeoJSON_DA
import random
import gdal
import uuid
import cv2
...
class SegmentationInstance:
    ...
    def prepare_training_data(self, shp_vector_layer, sheet_data, vector_data):
        ...
        # data augmentation
        points = ndshp2dict_DA(shp_vector_layer, lambda x: x.GetField('type') == "validation" or x.GetField('type') == "training")
        ...
        ###data augmentation
        dm.extract_region_DA(points, target_path, self.metadata["layer_names"], padding)
    ...
    def train_DA(self, data_location, batch_size = 20, epochs = 30, early_stopping_patience = 20, early_stopping_monitor = "val_binary_accuracy", upscale=False):
        ...
        ####data augmentation
        points = trim_GeoJSON_DA(data_location + "/points.geojson")
